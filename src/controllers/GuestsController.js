...
module.exports = {
    get: async (req, res) => {
        ...
        db.query(sql, (err, response) => {
            ...
            {
                let newUsers = response.map(item=>copyAndExcludes(item,["publicKey","privateKey","refreshToken","password","updated_date","updated_by","created_by"]))
    ...
    detail: async (req, res) => {
        ...
        let sql = `SELECT * FROM guests WHERE id=${userId}`
        console.log("sql",sql)
        db.query(sql, (err, response) => {
            if (err) res.send({result:"Failed",message: response})
            ...
            res.send({result:"okie",data:{user:userMain}})
        })
    },
    update: async (req, res) => {
        ...
        let sql = 'UPDATE guests SET ? WHERE id = ?'
    ...
    deletes: async (req, res) => {
        ...
        ids.forEach(function (item) {
            queries += `DELETE FROM users WHERE id = ${item.id} ; `
        });
        db.query(queries, (err, response) => {
            if (err) res.send({result:"Failed",message: err, queries:queries})
            ...
            //  console.log("passed",passed)
             ...
             else 
             {
                ...
                db.query(sql, [data], (err, user_inserted) => {
                    ...
                    {
                        console.log("user_register", user_inserted)
                        let data_user_role = {
                            user_id : user_inserted.insertId,
                            // todo: Lấy id của role PATIENT trong bảng roles
                            role_id : 4
                        } 
                        console.log("user_role",data_user_role)
                        let sql_user_role = 'INSERT INTO user_role SET ?'
                        db.query(sql_user_role, [data_user_role], (err, user_role_inserted) => {
                            if (err) res.send({message: err, result:"Có lỗi vui lòng liên hệ admin!"})
                            else
                            {
                                res.send({result:"okie",message:"Đăng ký tài khoản thành công"})
                            }
                        })
                    }
                })
